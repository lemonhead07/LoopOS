...
setup_repository() {
    ...
    # Check if we're already inside the LoopOS repo
    local script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
    if [ -f "$script_dir/CMakeLists.txt" ] && [ -d "$script_dir/src" ]; then
        print_info "Running from within LoopOS repository at $script_dir"
        REPO_DIR="$script_dir"
...
build_project() {
    ...
    if [ -z "$REPO_DIR" ]; then
        print_error "Repository directory not set"
        exit 1
    fi
...
${YELLOW}Useful Aliases (add to ~/.bashrc):${NC}
   alias loop='cd $REPO_DIR && ./loop'
   alias loopbuild='cd $REPO_DIR && ./scripts/build_unified.sh --auto'
...
main() {
    ...
    REPO_DIR=""  # Will be set by setup_repository
    ...
    if [ "$SKIP_BUILD" = "false" ]; then
        ...
        print_info "To build manually, run: cd $REPO_DIR && ./scripts/build_unified.sh --auto"
        echo ""
    fi
