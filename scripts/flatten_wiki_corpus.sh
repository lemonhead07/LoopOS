...
# Usage: ./scripts/flatten_wiki_corpus.sh [source_dir] [output_file] [max_files]
...
if [ ! -d "$SOURCE_DIR" ]; then
    echo "[flatten_wiki_corpus] Source directory not found: $SOURCE_DIR" >&2
    exit 1
fi
...
if [ "$TOTAL_FILES" -eq 0 ]; then
    echo "[flatten_wiki_corpus] No wiki_* shards found under $SOURCE_DIR" >&2
    rm -f "$TMP_FILE"
    exit 1
fi

echo "[flatten_wiki_corpus] Flattening $TOTAL_FILES files from $SOURCE_DIR"
...
for FILE in "${FILES[@]}"; do
    ...
    echo "[flatten_wiki_corpus] Appending $(basename "$FILE")" >&2
    cat "$FILE" >> "$TMP_FILE"
...
echo "[flatten_wiki_corpus] Wrote $(du -h "$OUTPUT_FILE" | awk '{print $1}') to $OUTPUT_FILE" >&2
if [ "$MAX_FILES" -gt 0 ]; then
    echo "[flatten_wiki_corpus] Included $COUNTER shard(s)" >&2
else
    echo "[flatten_wiki_corpus] Included all $COUNTER shard(s)" >&2
fi
