...
OptimizedFeedForward::OptimizedFeedForward(int d_model, int d_ff)
    : d_model_(d_model), d_ff_(d_ff) {
    initialize_weights();
}

void OptimizedFeedForward::initialize_weights() {
...
float OptimizedFeedForward::fast_gelu(float x) {
...
void OptimizedFeedForward::gelu_inplace(Math::IMatrix& x) {
    float* data = x.data();
    size_t size = x.size();
...
void OptimizedFeedForward::fused_linear_gelu(
...
std::unique_ptr<Math::IMatrix> OptimizedFeedForward::forward(const Math::IMatrix& input) {
    size_t seq_len = input.rows();
...
std::vector<std::unique_ptr<Math::IMatrix>> OptimizedFeedForward::forward_batched(
