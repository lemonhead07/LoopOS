...
OptimizedTransformerLayer::OptimizedTransformerLayer(
    int d_model, int num_heads, int d_ff, float dropout)
    : d_model_(d_model), num_heads_(num_heads), d_ff_(d_ff), dropout_(dropout) {
    
    attention_ = std::make_unique<OptimizedMultiHeadAttention>(d_model, num_heads);
    feedforward_ = std::make_unique<OptimizedFeedForward>(d_model, d_ff);
    norm1_ = std::make_unique<LayerNorm>(d_model);
    norm2_ = std::make_unique<LayerNorm>(d_model);
}

void OptimizedTransformerLayer::fused_residual_norm(
...
std::unique_ptr<Math::IMatrix> OptimizedTransformerLayer::forward(
    const Math::IMatrix& x,
...
std::vector<std::unique_ptr<Math::IMatrix>> OptimizedTransformerLayer::forward_batched(
...
// OptimizedTransformer implementation
OptimizedTransformer::OptimizedTransformer(
    ...
    for (int i = 0; i < num_layers; ++i) {
        layers_.push_back(
            std::make_unique<OptimizedTransformerLayer>(d_model, num_heads, d_ff));
    }
...
void OptimizedTransformer::initialize_embeddings() {
...
std::unique_ptr<Math::IMatrix> OptimizedTransformer::embed_tokens(
...
std::vector<std::unique_ptr<Math::IMatrix>> OptimizedTransformer::embed_tokens_batched(
...
std::unique_ptr<Math::IMatrix> OptimizedTransformer::create_causal_mask(int seq_len) {
...
std::unique_ptr<Math::IMatrix> OptimizedTransformer::forward(
    const std::vector<int>& token_ids) {
...
std::vector<std::unique_ptr<Math::IMatrix>> OptimizedTransformer::forward_batched(
    const std::vector<std::vector<int>>& token_ids_batch) {
    ...
    auto logits_batch = Math::OptimizedCPUMatrix::batch_matmul(normed_ptrs, proj_ptrs);
