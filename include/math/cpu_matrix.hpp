...
// CPU matrix with SIMD (AVX2/AVX512) and multithreading optimizations
...
private:
    ...
    void matmul_kernel_simd(const CPUMatrix& A, const CPUMatrix& B,
                           CPUMatrix& C, size_t i_start, size_t i_end) const;
    ...
    static void matmul_blocked(const CPUMatrix& A, const CPUMatrix& B,
                              CPUMatrix& C);
    
    // Transpose with cache blocking
    void transpose_blocked(CPUMatrix& result) const;
};
